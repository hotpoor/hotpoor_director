...
app.whenReady().then(async()=>{
 ...
 try{
  ...
  await js("document.querySelector('#dialogue-question').textContent='帮我把项目资料整理成研究提纲，保留来源与后续问题。';document.querySelector('#dialogue-compose').requestSubmit()");
  ...
  await capture('dialogue-workspace.png');
  await js("document.querySelector('#dialogue-model-trigger').click()");
  await capture('dialogue-model-picker.png');
  await js("document.querySelector('#dialogue-model-trigger').click()");
  ...
  await js("document.querySelector('#dialogue-run-mode').value='agent';document.querySelector('#dialogue-run-mode').dispatchEvent(new Event('change'));document.querySelector('#dialogue-question').textContent='整理当前项目目录，汇总 Markdown 文档。';document.querySelector('#dialogue-compose').requestSubmit()");
