...
def activity_svg(entries):
    ...
    parts = ['<svg xmlns="http://www.w3.org/2000/svg" width="720" height="174" viewBox="0 0 720 174" role="img" aria-labelledby="title desc">',
             ...
             '<rect width="720" height="174" rx="6" fill="#ffffff"/>',
             '<g font-family="Arial, PingFang SC, Microsoft YaHei, sans-serif" fill="#1f2328">',
             f'<text x="14" y="21" font-size="14" font-weight="600">{sum(period.values())} 条开发记录 · 过去一年</text>',
             f'<text x="702" y="21" text-anchor="end" font-size="10" fill="#59636e">{start.isoformat()} — {latest.isoformat()}</text>']
    for row, label in [(1, '周一'), (3, '周三'), (5, '周五')]:
        parts.append(f'<text x="14" y="{61 + row * 12}" font-size="10" fill="#59636e">{label}</text>')
    for offset in range(365):
        day = start + timedelta(days=offset)
        index = (day - grid_start).days
        column, row = divmod(index, 7)
        x, y = 50 + column * 12, 54 + row * 12
        count = counts.get(day.isoformat(), 0)
        if day.day == 1 or (offset == 0 and day.day <= 10):
            parts.append(f'<text x="{x}" y="44" font-size="10" fill="#59636e">{day.month}月</text>')
        parts += [f'<g><title>{day.isoformat()}：{count} 条开发记录</title>',
                  f'<rect data-date="{day.isoformat()}" data-count="{count}" x="{x}" y="{y}" width="9" height="9" rx="2" fill="{colors[level(count)]}" stroke="#d1d9e0" stroke-width="0.5"/>', '</g>']
    parts += [f'<text x="14" y="160" font-size="10" fill="#59636e">{len(period)} 个活跃日期 · 开发记录</text>',
              '<text x="410" y="160" font-size="10" fill="#59636e">少</text>']
    for i, (color, label) in enumerate(zip(colors, ['0', '1–3', '4–7', '8–14', '15+'])):
        x = 433 + i * 51
        parts += [f'<rect x="{x}" y="151" width="9" height="9" rx="2" fill="{color}" stroke="#d1d9e0" stroke-width="0.5"/>',
                  f'<text x="{x + 13}" y="159" font-size="10" fill="#59636e">{label}</text>']
    parts += ['<text x="700" y="160" font-size="10" fill="#59636e">多</text>', '</g>', '</svg>']
    return '\n'.join(parts) + '\n'
