...
app.whenReady().then(async()=>{
 ...
 try{
  ...
  if(!await js("document.querySelector('.dialogue-answer').classList.contains('dialogue-answer-collapsed') && document.querySelector('.dialogue-answer').getBoundingClientRect().height>=240 && document.querySelector('.dialogue-answer').getBoundingClientRect().width>=520 && getComputedStyle(document.querySelector('.dialogue-answer')).resize==='both' && document.querySelector('.dialogue-answer').scrollHeight>=document.querySelector('.dialogue-answer').clientHeight && document.querySelector('.dialogue-toggle-answer').textContent==='展开' && document.querySelector('.dialogue-toggle-answer').getAttribute('aria-expanded')==='false' && document.querySelector('.dialogue-icon-button').getAttribute('aria-label')==='复制回答'"))throw Error('Compact answer actions or minimum collapsed height failed');
  ...
  if(!await js("(()=>{const v=JSON.parse(localStorage.getItem('dialogue-card-sizes'));return Object.values(v).some(x=>x.width===610&&x.height===280)})()"))throw Error('Per-answer card size was not persisted');
  await js("document.querySelector('.dialogue-toggle-answer').click()");
