...
维护改动时使用北京时间 `## YYYY-MM-DD HH:mm · 事项` 记录当前时间，不推测旧事项的完成时刻；将最新记录插入 [开发日志](DEVELOPMENT_LOG.md) 的历史记录之前，保持日期倒序；运行 `python3 scripts/sync-development-history.py` 同步生成从早到晚的 [开发传记](DEVELOPMENT_HISTORY.md)，提交前用 `--check` 检查两份记录一致。需要更新 README 界面截图时，按 [截图来源与更新步骤](docs/screenshots/README.md) 运行隔离界面脚本并注明模拟数据范围。提交、推送与目标分支遵循当前使用者的授权，不把本文视为自动发布或重置数据的许可。
