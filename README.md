...
Electron 桌面端，内置 Python/Tornado 后端和 PostgreSQL，提供私有项目列表、无限画布、素材管理，以及通过 ComfyUI / service-inference 进行图片和视频生成，并支持云存储直传与评论标注。
...
以下为 **2026-09-15 在 macOS 上运行本仓库 Electron 界面时的实际截图**，使用独立测试账号、临时 PostgreSQL 和模拟云服务响应。截图展示已实现的界面与交互；Logo 图片、纯色视频均为测试素材，不代表模型生成质量或真实云服务联通结果。可点击图片查看原始尺寸，来源与更新步骤见 [截图说明](docs/screenshots/README.md)。
...
工作台顶部点击「对话模式」，选择已启用的 service-inference AK 和语言模型后提问。点击「刷新可用模型」直接查询该 AK 的 `/v1/models`，仅展示 `llm` 类型；每次提交再校验可用模型。可在同一对话中切换 AK 与模型，GPT-6 默认使用 Responses，其他模型默认使用 Chat Completions，也可手动选择接口。普通问答不携带工具调用参数。

对话支持「添加文件」、拖入文件和粘贴图片，每次最多 5 个文件、单个 10 MB、合计 20 MB。支持 PNG/JPEG/WebP、PDF、DOCX，以及 TXT/Markdown/CSV/JSON/代码等文本文件；Word 与文本提取文字，图片和 PDF 按接口作为原生输入，需所选模型支持。附件独立生成 UUID 并分片，绑定账号与对话，保存后可从历史记录下载；后续成功问答继续携带附件，最近 20 轮附件合计仍限制 20 MB。

每次创建对话和记录包都生成独立 UUID，通过各自 UUID 求余路由实体分片。对话保存 `first_id` / `last_id`，每包保存 25 轮问答，通过 `prev_id` / `next_id` 链接；满包创建新包并原子更新指针。完整历史保留，界面支持加载更早记录；模型接收最近 20 轮成功问答，最多 120000 字符，超过时提示新建对话。回答支持 Markdown、安全过滤及复制，并记录实际模型、AK 名称、时间与服务商返回的用量。

提交先保存问题，再后台请求模型并保存回答。相同请求 UUID 不会重复提交；断线或服务重启不会自动重发，需在服务控制台核对后点击「已核对中断请求，继续对话」。对话归创建账号私有，不加入项目分享或项目同步。密钥不写入对话或记录包。
