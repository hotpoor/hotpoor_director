...
- 触控板双指上下左右移动可平移画布，双指捏合可缩放，鼠标停在卡片上同样生效（textarea 内优先滚动文本，不触发画布手势）；也可拖动空白处平移、Ctrl+滚轮缩放画布。底栏「缩放灵敏度」提供 1×–20× 档位，默认 6×（为上一版速度的 3 倍），即时生效并保存在当前浏览器；客户端与线上分别保存。拖动标题移动卡片，四边及四角均可调整尺寸。底栏可适配全部卡片或恢复 100%。
