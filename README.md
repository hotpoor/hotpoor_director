# Hotpoor Director · 无限画布创作工作台

以项目 Dashboard 与无限画布为核心，在同一张画布上组织素材、连接参考、生成图片与视频、并排比较结果、标注审阅，再通过多时间轴编排和云端协作推进创作。AI 对话、资料阅读与本机代理执行提供辅助能力。桌面端使用 Electron，内置 Python / Tornado 后端并管理本机 PostgreSQL；云端工作站复用画布与协作功能。

本文按 **2026-09-23 仓库实现**整理。模型表表示客户端已接入的适配能力，实际可用模型由所选 AK 和服务端返回结果决定。

[项目与画布](#项目-dashboard-与无限画布) · [创作流程](#创作流程与功能导航) · [生成](#图片与视频生成) · [对比与审阅](#素材预览与评论审阅) · [时间轴](#视频时间轴) · [协作](#同步与协作) · [辅助对话](#对话与代理模式) · [快速开始](#快速开始) · [配置](#模型-ak费用与云存储) · [安装与开发](#安装与开发)

## 项目 Dashboard 与无限画布

登录后进入自己的项目列表。可创建或编辑主标题、副标题、描述和多张封面（第一张为主封面，最多 20 张）。项目内用独立卡片组织素材、生成任务与评论，将参考关系保留在画布上。

![无限画布：素材、评论与生成卡片通过参考线关联，下游展示引入素材](docs/screenshots/workspace.png)

```mermaid
flowchart LR
    A["导入素材"] --> B["连线引用"] --> C["图片 / 视频生成"]
    C --> D["PIN 对比与评论标注"] --> E["时间轴编排"] --> F["分享协作"]
```

[查看每日开发记录](DEVELOPMENT_LOG.md#每日工作记录分布) · [查看项目开发时间线](DEVELOPMENT_HISTORY.md#开发时间线)

- **自由组织**：平移缩放、拖动卡片、调整尺寸；触控板和鼠标均可操作。
- **连接参考**：素材、评论与生成卡片可以连线，下游直接复用上游资料和结果。
- **持续迭代**：同一项目混用本地与云端模型，保留生成历史、参数和对比结果，编辑自动保存。
...
以下按创作过程排列；AI 对话与本机代理放在画布相关能力之后。
...
在画布上添加图片或视频生成卡片，选择模型和创作模式，引用素材后提交。结果、参数与历史保留在卡片内，可继续作为下游参考。
...
4. 开始创作：创建项目，导入素材，添加图片 / 视频生成卡片；用连线组织参考，使用评论、PIN 和时间轴审阅结果。云端模型引用本地素材前，先配置云存储直传。
5. 开始对话：进入「对话模式」，选择模型并提问；代理执行、知识库和阅读偏好在左下角「设置」中配置。
...
每个项目使用服务端生成的 32 位 UUID，创建和更新时间采用数据库毫秒时间戳。
