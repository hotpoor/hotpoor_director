...
以项目 Dashboard 与无限画布为核心，把图片与视频生成、素材审阅、时间轴编排、AI 对话、资料阅读和本机代理执行放进同一个工作台。桌面端使用 Electron，内置 Python / Tornado 后端并管理本机 PostgreSQL；云端工作站复用画布与协作功能。

本文按 **2026-09-23 仓库实现**整理。模型表表示客户端已接入的适配能力，实际可用模型由所选 AK 和服务端返回结果决定。

[项目与画布](#项目-dashboard-与无限画布) · [功能总览](#功能总览) · [快速开始](#快速开始) · [对话与代理](#对话与代理模式) · [生成](#图片与视频生成) · [素材与审阅](#素材预览与评论审阅) · [时间轴](#视频时间轴) · [同步与协作](#同步与协作) · [配置](#模型-ak费用与云存储) · [安装与开发](#安装与开发) · [技术栈](#技术栈与服务来源)
