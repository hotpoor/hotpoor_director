        ...
        async with self.settings['comfy_lock']:
            if connection != self.settings['comfy_connection']:
                async with self.jobs.connection() as conn:
                    active = await (await conn.scan("SELECT 1 FROM entities WHERE body->>'kind'='generation' AND body->>'provider' IS DISTINCT FROM 'service-inference' AND body->>'status' IN ('submitting','queued','running','stopping') LIMIT 1")).fetchone()
                if active:
