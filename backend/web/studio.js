(() => {
  ...
  function renderCard(card) {
    ...
    const allCredentials=state.inferenceCredentials||{keys:[]};
    const credentials={...allCredentials,keys:allCredentials.keys.filter(p=>(allCredentials.enabled_key_ids||[]).includes(p.id))};
    if(!card.credential_id && credentials.active_key_id)card.credential_id=credentials.active_key_id;
