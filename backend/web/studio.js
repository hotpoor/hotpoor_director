(() => {
  ...
  function draft(card) {
    ...
    if(card.model && card.drafts[card.mode]?.model && card.drafts[card.mode].model!==card.model){
      ...
      for(const k of ['width','height','duration','size','resolution','output_format','optimize_mode','quality','background','n','turbo_mode','standard_steps'])card.drafts[card.mode][k]=defaults[k];
  ...
  async function useMaterial(card,key){
    ...
    if(model?.provider==='service-inference'){
      if(material.storage==='cloud'){
        ...
        const single=key==='first_frame',urls=(d[key]||'').split(/\s+/).filter(Boolean),limit=single?1:key==='image_urls'?(model.image_ref_limit||model.ref_limit):3;
  ...
  function parameterSummary(body){const p=body.params;if(body.provider==='service-inference')return body.type==='image'?`分辨率 ${p.size}`:`${p.resolution} · ${p.ratio} · ${p.duration} 秒`;return `${p.width} × ${p.height} · ${p.steps} 步${body.model==='minimax-h3-ref2va'?(p.turbo_mode?' · Turbo':' · 标准'):''} · seed ${p.seed}`;}
  function cloudFields(card,model,d){
    ...
    if(card.mode!=='text'){
      ...
      else html+=refs('image_urls',`参考图片 URL · 最多 ${model.image_ref_limit||model.ref_limit} 张（依填写顺序编号）`);
    ...
    if(model.remote_model==='minimax-h3'&&card.mode==='reference')html+=`<p class="mode-note">混合参考最多12项；视频、音频各最多3段，各类总时长不超过15秒，单段2–15秒。音频限MP3/WAV、每个≤15MB，须搭配图片或视频。保留音色时请写明：&lt;Audio 1&gt;仅参考音色，不复用原台词。服务端负责校验远程素材。</p>`;
  ...
  $('#canvas-world').addEventListener('input',event=>{
    ...
    if(event.target.dataset.field && event.target.dataset.field!=='model'){
      const key=event.target.dataset.field,d=draft(card);
      if(key==='turbo_mode'){
        if(event.target.checked){d.standard_steps=d.steps;d.steps=4;}
        else d.steps=d.standard_steps||20;
      }
      d[key]=event.target.type==='checkbox'?event.target.checked:['model','prompt','negative_prompt','size','resolution','ratio','image_urls','video_urls','audio_urls','first_frame','last_frame','output_format','optimize_mode','quality','background'].includes(key)?event.target.value:Number(event.target.value);
      changed();updateGenerationSummary(card);if(['image_urls','video_urls','audio_urls','first_frame','last_frame'].includes(key))refreshReferenceMentions(card,el,key);if(['width','height'].includes(key))updateSizeFeedback(card);
    }
  });
  $('#canvas-world').addEventListener('change',async event=>{
    const el=event.target.closest('[data-card]');if(!el)return;const card=cardById(el.dataset.card);
    if(event.target.dataset.field==='turbo_mode'){const top=el.querySelector('.card-content').scrollTop;renderCard(card);el.querySelector('.card-content').scrollTop=top;el.querySelector('[data-field=turbo_mode]').focus({preventScroll:true});return;}
  ...
  async function addCloudReferences(card,key,files){
    ...
    const single=['first_frame','last_frame'].includes(key),limit=single?1:key==='image_urls'?(spec.image_ref_limit||spec.ref_limit):3;
