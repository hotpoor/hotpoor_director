(() => {
  ...
  function edit(id,open=true){editing=id||'';const p=config.keys.find(k=>k.id===editing);form.elements.name.value=p?.name||'';key.value='';binding(p?.management_key_id);document.querySelector('#clear-inference').disabled=!p;models(p?.models,p?.models_loaded);placeEditor(editing,open);}
