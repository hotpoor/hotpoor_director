...
已将 16 个完整 safetensors 权重及 11 个 YAML 配置复制到 `I:/AI/ComfyUI/models`，共 138,083,237,766 字节（约 138.08 GB）。逐文件在复制源数据时计算 SHA256，再完整读取目标数据对比，27 个文件全部一致。下载分块未复制，`J:/codex_projects/ComfyUI/models` 原件保留。
...
如需回退，在生成队列空闲时从 extra_model_paths.yaml 移除 director_ssd 这一项并重启 ComfyUI，即可恢复原 J 盘模型优先级。不要删除其他模型路径配置。
